...
window.PS.util = {
    md5Hex: function (s) {
        return encHex.stringify(MD5(s));
    },
    sha1Hex: function (s) {
        return encHex.stringify(SHA1(s));
    },
    sha256Hex: function (s) {
        return encHex.stringify(SHA256(s));
    },
    encodeBase64: function (s) {
        return encBase64.stringify(encUtf8.parse(s));
    },
    decodeBase64: function (s) {
        return encBase64.parse(s).toString(encUtf8);
    }
};
...
window.PS.form = {
     ...
     *
     * @method _bindKey
     * @param {Number} PS.form.keycode 键盘码
    ...
    bindEscKey: function (element, callback) {
        this._bindKey(this.keycode.EPS, element, callback);
        return this;
    },
    /**
    ...
    _serializeInputs: function (inputs) {
        ...
        for (var i = inputs.length - 1; i >= 0; i--) {
            ...
            var dataType = input.attr('data-sc-type');
            if (value && dataType) {
                switch (dataType) {
                    case 'int':
...
window.PS.ajax = {
    _ajax: function (type, target, param, dataType, contentType) {
        var options = {
            type: type,
            async: true,
            url: target,
        ...
        var isGetOrDelete = type == 'GET' || type == 'DELETE';
        if (!isGetOrDelete && options.dataType == 'json') {
            options.contentType = contentType || 'application/json; charset=utf-8';
        }
        if (isGetOrDelete) {
            var url = target;
            if (target.indexOf('?') == -1) {
            ...
            options.url = url + arr.join('&');
        } else if (options.contentType && options.contentType.indexOf('/json') != -1) {
    ...
    post: function (target, param, dataType, contentType) {
        return this._ajax('POST', target, param, dataType, contentType);
    },
    get: function (target, param, dataType, contentType) {
        return this._ajax('GET', target, param, dataType, contentType);
    },
    put: function (target, param, dataType, contentType) {
        return this._ajax('PUT', target, param, dataType, contentType);
    },
    delete: function (target, param, dataType, contentType) {
        return this._ajax('DELETE', target, param, dataType, contentType);
    }
};
